...
bucket = 'sa-doc-upload' #'test-speechcapture'
...
def upload(request):
    ...
    s3_client.upload_file(Filename=os.path.join(os.getcwd(), fileName), Bucket=bucket, Key=fileName)
...
def transcript_backend(request, fileName):
    ...
    try:
        ...
        s3_client.get_object(Bucket=bucket, Key=newFileName)
    #file is not found so the transcription is not readily available thus we need to create the transcription job
        ...
        job_uri = "https://s3-us-west-2-amazonaws.com/" + bucket + "/" + fileName
        ...
        transcribe_client.start_transcription_job(
            ...
            OutputBucketName= bucket,# OutputBucketName='test-speechcapture', #the S3 bucket to output the transcription file to(the file will be the same name as the job_name appended with .json)
    ...
    result = s3_client.get_object(Bucket= bucket, Key= newFileName)
    ...
    s3_client.upload_file(Filename=os.path.join(os.getcwd(), textFileName), Bucket=bucket, Key=textFileName, ExtraArgs={'ACL':'public-read'})
...
<<<<<<< HEAD
    ...
    if s3_client.get_object(Bucket=bucket, Key=(job_name + 'Analysis' + '.json')):
        # result = s3_client.get_object(Bucket='test-speechcapture', Key=(job_name + 'Analysis' + '.json'))
        result = s3_client.get_object(Bucket=bucket, Key=(job_name + 'Analysis' + '.json'))

        return render(request, 'webapp/analysis.html', {'data': result})
    else:
        # result = s3_client.get_object(Bucket='test-speechcapture', Key=(job_name + '.txt'))
        result = s3_client.get_object(Bucket=bucket, Key=(job_name + '.txt'))
        ...
        try:
            ...
            while True:
                try: 
                    ...
                    print("Not ready yet...")
                    time.sleep(5)
>>>>>>> dd8e65055e433b765caba73be259cb7ec5b0e90e
